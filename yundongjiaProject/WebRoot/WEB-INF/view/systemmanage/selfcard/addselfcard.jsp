...
<script type="text/javascript">
	...
	function subForm(){
		...
		$.ajax({
			url : "<%=path%>/yundongjia/insertSelfCardType",
			type: 'POST',  
            data: data,
            dataType: 'JSON',  
            async : false,
			...
			success : function(data){
				if(data.result){
					var maincontent = $('.index-right');
					maincontent.load('<%=path%>/yundongjia/selfCardTypeDetail?cardid=' + data.id);
					alert(data.message);
				}else{
					alert(data.message);
				}
...
<body>
	<div class="main-content" >
		<div class="page-header">
			当前位置：
			<a href="javascript:void(0)" status="0" >新增跟人卡类型</a>
		</div>
		<div>
			 <form id="formInfo" name="form1"   onsubmit="return false;"  method="post">
					...
					<div class="col-md-12">
						...
						<div class="col-md-3 padding0L form-group">	
							<input type="radio" name="iscashback"  checked="checked" value="0" >不返现
							<input type="radio" name="iscashback"  value="1"   style="margin-left: 10px;">全额返现
						</div>  			
					</div>
					<div class="col-md-8">
