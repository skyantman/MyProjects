...
<body>
	<div class="main-content reManage" id="reManage">
		<div class="page-header">
				...
				&nbsp;>&nbsp;
			<a href="javascript:void(0)" >企业卡类型详情</a>
			</div>
						...
						<div class="main-content-item" style="padding-left: 30px">
							<table>
								...
								<tr>
									...
									<td>
										<div class="form-group">
											<label for="">${cardType.sportscount }次</label>
										</div>
									</td>
								</tr>
							</table>
							...
							<table class="table" rules="cols" frame="box">
								...
								<c:forEach items="${cardDet.list }" var="cd">
										<tr>
											...
											<td>${cd.discount}</td>
											<td>${cd.money}</td>
											<td>${cd.sportsCount}</td>
											<td>${cd.sportsCount-cd.surplusSportsCount}</td>
											...
											<td>
												...
												<a href="javascript:void(0)" cardId ="${cd.orderId }"  enterId="${cd.enterprise_id }"  onclick="memberList(this);" class=" ">用户列表</a>
											</td>
										</tr>
								</c:forEach>		
								</tbody>
	...
	if(re == 1 || re == "1"){
		$.ajax({
			...
			success : function(data){
				if(data.result){
					var maincontent = $('.index-right');
					maincontent.load('<%=path%>/yundongjia/enterpriseCardTypeDetail?cardid=${cardType.cardid}&status=1');
					alert(data.message);
					//关闭模态框
...
function search(page){
	var info = $("#search").val();
	var maincontent = $('.index-right');
	maincontent.load('<%=path%>/yundongjia/enterpriseDetail?cardid=${cardType.cardid}&info='+info + '&page=' + page);
}
...
function detailCard(obj){
	...
	var maincontent = $('.index-right');
	maincontent.load('<%=path%>/yundongjia/enterpriseCardDetail?cardtypeid=${cardType.cardid}&cardid='+cardId+'&status=0');
}

function memberList(obj){
	var enterId = $(obj).attr("enterId");
	var cardId = $(obj).attr("cardId");
	var t= new Date().valueOf();
	$(".index-right").load('<%=path%>/yundongjia/detailenterprise?cardId='+ cardId +'&enterpriseCardId=${cardType.cardid}&tab=3&id='+enterId+'&pageNum=1&_time='+t);
}
</script>
</body>
</html>