...
<script type="text/javascript">
	function goback(info){
		...
		if(info == 1){
			...
			maincontent.load('<%=path %>/yundongjia/enterpriseCardTypeDetail?cardid=${cardTypyId}&status=1');
		}else if(info == 3){
	...
	function toback(num){
		...
		if(0 == num){
			url='<%=path%>/yundongjia/listenterprise?pageNum=1&_time='+t;
		}else if(1 == num){
			var cid = ${card.orderId};//卡id
			var eid = ${card.enterpriseId};  //企业id
			url = '<%=path%>/yundongjia/detailenterprise?cardId='+cid+'&id='+eid+'&tab=1&_time='+t;
		}
		tobackDemo(url);
	}
</script>
		...
		<div class="page-header">
			...
			<c:if test="${condition != 1 }">
				<c:if test="${position == 0}">
				   <a href="javascript:void(0)" onclick="goback(1)">企业卡类型列表</a>
					&nbsp;>&nbsp;
					<a href="javascript:void(0)"  onclick="goback(2)">企业卡类型详情</a>
				</c:if>
				<c:if test="${position == 1}">
					<a href="javascript:toback(0);">企业列表</a>
					&nbsp; &gt; &nbsp;
					<a href="javascript:toback(1);">企业详情</a>
				</c:if>
				&nbsp;>&nbsp;
				<a href="javascript:void(0)" >企业卡详情</a>
			 </c:if>
			 <c:if test="${condition == 1 }">
				<a href="javascript:void(0)"  onclick="goback(3)">用户列表</a>
					...
					<a href="javascript:void(0)"  onclick="goback(4)">用户详情</a>
						&nbsp;>&nbsp;
					<a href="javascript:void(0)" >企页卡详情</a>
			</c:if>
		</div>	
						...
						<div class="main-content-item" style="padding-left: 30px">
							<table>
								...
								<tr>
									...
									<td>
										<div class="form-group">
											<label for="">${card.enterpriseName }</label>
										</div>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>
										<div class="form-group">
											<label for="">${card.totalMoney }元</label>
										</div>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>
										<div class="form-group">
											<label for="">${card.sportsCount }次</label>
										</div>
									</td>
								</tr>
								<tr>
									...
									<td>
										<div class="form-group">
											<label for="">${card.surplusSportsCount }次（已使用${card.sportsCount-card.surplusSportsCount }次）</label>
										</div>
									</td>
								</tr>
								<tr>
...
function search(page){
	var info = $("#search").val();
	var maincontent = $('.index-right');
	maincontent.load('<%=path%>/yundongjia/searchOrder?status=1&cardtypeid=${cardTypyId}&cardid=${card.cardId}&info='+ info +'&pageNum='+page);
}
	...
	$.ajax({
		...
		success:function(data){
			if(data > 0){
		        ...
		        var maincontent = $('.index-right');
				maincontent.load('<%=path%>/yundongjia/enterpriseCardDetail?cardtypeid=${cardTypyId}&cardid=${card.cardId}&status=1');
			}
		}
	});
}
...
function orderDetail(orderId,type){
	...
	//订单类型，订单ID
var t=new Date().valueOf();
	$(".index-right").load('<%=path%>/yundongjia/getorderbyid?nav=2&cardtypeid=${cardTypyId}&cardid=${card.cardId}&orderId='+orderId+'&orderType='+orderType+'&_time='+t);
}
</script>
</body>
</html>